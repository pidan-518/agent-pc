...
class Success extends React.Component {
      ...
      .then(res => {
        if (res.data.code === 0) {
          ...
          this.setState(
            ...
            () => {
              this.getDrawAdminList({ current: 1, len: 10, state: 2 })
            }
          )
        }
      })
  ...
  handleExportExcel = () => {
    this.getDrawAdminList(
      { current: 1, len: this.state.totalNum, state: 2, ...this.state.searchParms },
      'all'
    )
  }
        ...
        if (res.data.code === 0) {
          ...
          if (type === 'all') {
            ...
            this.setState(
              ...
              () => {
                qee(columns, this.state.alldata, '提现驳回')
              }
            )
          } else {
            this.setState({
  ...
  handleChangePageSize = e => {
    ...
    if (pageSize > this.state.pageSize) {
      this.getDrawAdminList({ current: 1, len: pageSize, state: 2, ...this.state.searchParms })
    }
  ...
  handleChangePage = (page, pageSize) => {
    ...
    this.getDrawAdminList({ current: page, len: pageSize, state: 2, ...params })
  }
  ...
  onFinish = values => {
    ...
    this.getDrawAdminList({ current: 1, len: this.state.pageSize, state: 2, ...search })
  }

  // 表单重置
  onReset = () => {
    this.searchform.current.resetFields()
    this.getDrawAdminList({ current: 1, len: this.state.pageSize, state: 2 })
    this.setState({
      searchParms: null,
    })
  }
    ...
    return (
      <div id="extraction">
        <div className="header">
          ...
          <span className="headerName">驳回记录</span>
        </div>
        {/* 搜索框 */}
        <Form
          ref={this.searchform}
